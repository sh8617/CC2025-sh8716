...
function draw() {
  ...
  if (str.length > 0) {
    potentialmeter = sensorValues[0];
    photoCell = sensorValues[1]
  }

  text("potentialmeter:" + potentialmeter, 10,50);
  text("photoCell:" + photoCell, 10,70);
  let circleD = map(potentialmeter,0,1023,0,width);
  let hue = map(potentialmeter,0,1023,0,360);
  let brightness = map(photoCell,0,1023,0,360);
